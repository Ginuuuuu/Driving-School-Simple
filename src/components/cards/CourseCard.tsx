  ...
  return (
    <div
      className={`group flex flex-col justify-between rounded-2xl sm:rounded-3xl bg-white border transition-all duration-300 hover-lift ${
        course.popular
          ? 'border-[#42B7A7] shadow-md ring-2 ring-[#42B7A7]/20'
          : 'border-[#C2D3D0] shadow-sm'
      }`}
    >
      {/* Card Header & Badges */}
      <div className="p-4 sm:p-6 pb-3 sm:pb-4">
        ...
        {/* Title */}
        <h3 className="text-base sm:text-xl font-bold font-display text-[#26423E] group-hover:text-[#42B7A7] transition-colors leading-snug">
        ...
        {/* Summary Description */}
        <p className="mt-1.5 sm:mt-2.5 text-xs sm:text-sm text-[#56776A] leading-relaxed line-clamp-2">
          {course.summary}
        </p>

        {/* Key Stats Bar */}
        <div className="mt-3 sm:mt-4 pt-2.5 sm:pt-3 border-t border-[#C2D3D0]/60 grid grid-cols-2 gap-2 text-[0.72rem] sm:text-xs text-[#26423E]">
          <div className="flex items-center gap-1.5 font-semibold">
            <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#42B7A7] shrink-0" />
            <span className="truncate">{course.durationHours} hrs Practical</span>
          </div>
          <div className="flex items-center gap-1.5 font-semibold">
            <Car className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#42B7A7] shrink-0" />
        ...
        <div className="mt-3 sm:mt-4 space-y-1 sm:space-y-1.5">
          {course.whatIncluded.slice(0, 3).map((item, idx) => (
            <div key={idx} className={`flex items-start gap-1.5 sm:gap-2 text-[0.72rem] sm:text-xs text-[#56776A] ${idx === 2 ? 'hidden sm:flex' : 'flex'}`}>
      ...
      <div className="p-3.5 sm:p-6 pt-3 sm:pt-4 border-t border-[#C2D3D0] bg-[#EDEFF4]/70 rounded-b-2xl sm:rounded-b-3xl flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl sm:text-2xl font-extrabold text-[#26423E]">
              ₹{course.price.toLocaleString('en-IN')}
            </span>
            {course.originalPrice && (
              <span className="text-[0.7rem] sm:text-xs text-[#56776A] line-through">
                ₹{course.originalPrice.toLocaleString('en-IN')}
              </span>
            )}
          </div>
          <span className="text-[0.65rem] sm:text-[0.7rem] text-[#56776A] font-medium block truncate">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <Link
            ...
            className="text-xs font-bold text-[#26423E] hover:text-[#42B7A7] px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg hover:bg-[#C2D3D0]/40 transition-colors"
          >
            Details
          </Link>
          <Button
