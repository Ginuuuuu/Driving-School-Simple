            ...
            >
              ...
              >
                ...
                <div className="block md:hidden pr-2">
                  <div className="flex items-center justify-between gap-1 mb-0.5">
                    <span className="font-mono text-[0.68rem] uppercase tracking-wider font-bold text-[#082B4C]">
                      Stage {step.stepNumber}
                    </span>
                    {step.approxDuration && (
                      <span className="text-[0.65rem] text-[#6B7280] font-mono">
                        {step.approxDuration}
                      </span>
                    )}
                  </div>
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm font-bold font-display text-[#202B33] leading-snug break-words max-w-[calc(100vw-85px)] group-hover:text-[#082B4C] transition-colors">
                      {step.title}
                    </h3>
                    {!isCompactPreview && (
                      <ChevronDown
                        className={`w-4 h-4 shrink-0 text-[#6B7280] transition-transform duration-200 mt-0.5 ${
                          isExpanded ? 'rotate-180 text-[#082B4C]' : ''
                        }`}
                      />
                    )}
                  </div>
                ...
                <div className="hidden md:flex items-center gap-2.5">
                  <span className="font-mono text-xs uppercase tracking-wider font-bold text-[#082B4C]">
                    Stage {step.stepNumber}
                  </span>
                  <span className="text-base sm:text-lg font-semibold font-display text-[#202B33] group-hover:text-[#082B4C] transition-colors">
                    {step.title}
                  </span>
                  {step.approxDuration && (
              ...
              <TimelineDot
                ...
                className={`transition-all duration-300 ${
                  isCurrent
                    ? 'ring-4 ring-[#F4C400]/30 scale-125 border-[#F4C400] bg-[#F4C400] text-[#082B4C] shadow-sm'
                    : isDone
                    ? 'border-[#082B4C] bg-[#082B4C] text-white shadow-xs'
              ...
              <TimelineContent side="right" className="pt-1 max-w-3xl">
                ...
                <div className="hidden md:block space-y-3 pb-8">
                  ...
                  {/* Summary text */}
                  <p className="text-sm text-[#202B33] leading-relaxed">
                    {step.summary}
                  </p>
                  ...
                  {!isCompactPreview && step.requiredDocuments && step.requiredDocuments.length > 0 && (
                    <div className="pt-2 space-y-1.5">
                      <span className="text-xs font-bold text-[#202B33] flex items-center gap-1.5 uppercase tracking-wider">
                        <FileText className="w-3.5 h-3.5 text-[#082B4C]" />
                        Key Requirements:
                      </span>
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs text-[#6B7280] pl-1">
                    ...
                    <div className="pt-2 space-y-1.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {step.rtoTrackManeuvers.map((drill, drillIdx) => (
                          <span
                            key={drillIdx}
                            className="px-2.5 py-1 rounded-lg bg-[#F4C400]/15 border border-[#F4C400]/30 text-[0.72rem] text-[#202B33] font-medium"
                          >
                            {drill}
                          </span>
                        ))}
                  ...
                  {!isCompactPreview && step.instructorProTip && (
                    <div className="pt-2 flex items-start gap-2 text-xs text-[#202B33] bg-[#F4C400]/15 border border-[#F4C400]/30 p-2.5 rounded-xl">
                  ...
                  <div className="pt-2 flex items-center gap-4 text-xs">
                    {step.rtoPortalUrl && (
                      <a
                        ...
                        className="inline-flex items-center gap-1 text-[#082B4C] font-semibold hover:underline"
                      >
                    ...
                    {onOpenBookingModal && (
                      <button
                        ...
                        className="text-[#082B4C] font-bold hover:text-[#061F36] underline transition-colors cursor-pointer"
                      >
                  ...
                  <div className="block md:hidden">
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        ...
                        >
                          <p className="text-xs text-[#202B33] leading-relaxed">
                            {step.summary}
                          </p>
                          ...
                          {step.requiredDocuments && step.requiredDocuments.length > 0 && (
                            <div className="p-2.5 rounded-xl bg-white/90 border border-[#E5E7EB] space-y-1">
                              <span className="text-[0.68rem] font-bold text-[#202B33] uppercase tracking-wider flex items-center gap-1">
                                <FileText className="w-3 h-3 text-[#082B4C]" />
                                Requirements:
                              </span>
                          ...
                          {step.instructorProTip && (
                            <div className="p-2.5 rounded-xl bg-[#F4C400]/15 border border-[#F4C400]/30 text-xs text-[#202B33] flex items-start gap-2">
                              <Lightbulb className="w-3.5 h-3.5 text-[#082B4C] shrink-0 mt-0.5" />
                          ...
                          <div className="pt-1 flex flex-col gap-2">
                            {step.rtoPortalUrl && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1 text-xs text-[#082B4C] font-bold hover:underline"
                              >
                            ...
                            {onOpenBookingModal && (
                              <Button
                                ...
                                size="sm"
                                onClick={onOpenBookingModal}
                                className="w-full justify-center shadow-xs text-[#082B4C]"
