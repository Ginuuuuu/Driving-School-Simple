...
export const FAQ: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-10 space-y-6 sm:space-y-12">
      ...
      <div className="space-y-3 sm:space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 sm:pl-12 pr-3.5 sm:pr-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-slate-200 bg-white text-xs sm:text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none shadow-xs"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 sm:gap-3">
          ...
          <div className="flex items-center gap-2 text-[0.7rem] sm:text-xs font-semibold text-slate-500">
            <button onClick={handleExpandAll} className="hover:text-emerald-700 underline">
              Expand All
            </button>
            <span>•</span>
            <button onClick={handleCollapseAll} className="hover:text-emerald-700 underline">
              Collapse All
            </button>
          </div>
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          {filteredFAQs.map((faq) => {
            ...
            return (
              <div
                key={faq.id}
                className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white shadow-xs overflow-hidden transition-all"
              >
                <button
                  onClick={() => toggleItem(faq.id)}
                  className="w-full p-3 sm:p-5 text-left flex items-start justify-between gap-3 hover:bg-slate-50 transition-colors"
                >
                ...
                {isOpen && (
                  <div className="px-3.5 sm:px-5 pb-3.5 sm:pb-5 pt-1 border-t border-slate-100 bg-slate-50/50 animate-in fade-in duration-150">
      ...
      <section className="p-4 sm:p-8 rounded-2xl sm:rounded-3xl bg-slate-900 text-white border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
        ...
        <Button
          ...
          href={whatsappUrl}
          isExternal
          className="shrink-0 shadow-md text-xs sm:text-sm py-2 sm:py-2.5 w-full sm:w-auto justify-center"
