...
export const Dashboard: React.FC = () => {
  ...
  const quickStats = [
    { label: 'Active Courses', count: courses.length, to: '/admin/courses', icon: BookOpen, color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
    { label: 'Licence Steps', count: roadmap.length, to: '/admin/roadmap', icon: MapPin, color: 'text-amber-700 bg-amber-50 border-amber-200' },
    { label: 'Instructors', count: instructors.length, to: '/admin/instructors', icon: Users, color: 'text-blue-700 bg-blue-50 border-blue-200' },
    ...
    { label: 'Testimonials', count: testimonials.length, to: '/admin/testimonials', icon: MessageSquare, color: 'text-pink-700 bg-pink-50 border-pink-200' },
  ...
  const editorCategories = [
    {
      ...
      description: 'Manage curriculum, session breakdowns, pricing packages, and add-on services.',
      links: [
        { name: 'Courses & Curriculum', to: '/admin/courses', icon: BookOpen, count: courses.length },
        { name: 'Pricing & Add-ons', to: '/admin/pricing', icon: CreditCard, count: pricing.packages.length },
      ],
    },
    {
      ...
      links: [
        { name: 'Licence Roadmap', to: '/admin/roadmap', icon: MapPin, count: roadmap.length },
        { name: 'Instructors Roster', to: '/admin/instructors', icon: Users, count: instructors.length },
        { name: 'Learner Testimonials', to: '/admin/testimonials', icon: MessageSquare, count: testimonials.length },
      ],
    },
    {
      title: 'Site Identity & Channels',
      description: 'Configure brand settings, WhatsApp / phone support channels, FAQs, and SEO tags.',
      links: [
        { name: 'General Settings', to: '/admin/settings', icon: Settings },
        { name: 'FAQ Manager', to: '/admin/faqs', icon: HelpCircle, count: faqs.length },
        { name: 'Contact & Branches', to: '/admin/contact', icon: Phone },
        { name: 'SEO & Metadata', to: '/admin/seo', icon: Search },
        { name: 'Legal Policies', to: '/admin/legal', icon: Shield },
      ],
    },
  ];
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 rounded-3xl bg-slate-900 text-white border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2 relative z-10">
          ...
          <p className="text-xs sm:text-sm text-slate-300 max-w-xl leading-relaxed">
            Manage your entire website content, courses, roadmap stages, instructors, pricing, and contact channels in real time.
          </p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold font-display uppercase tracking-wider text-slate-500">
          ...
          {quickStats.map((stat, idx) => {
            ...
            return (
              <Link
                key={idx}
                to={stat.to}
                className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-xs hover:shadow-md hover:border-slate-300 transition-all space-y-2 group"
              >
                <div className={`w-8 h-8 rounded-xl border flex items-center justify-center ${stat.color}`}>
                  <Icon className="w-4 h-4" />
                </div>
                <div>
                  <div className="text-2xl font-bold font-display text-slate-900 group-hover:text-emerald-700 transition-colors">
                    {stat.count}
                  </div>
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-slate-100 border border-slate-200/90 text-xs text-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1 max-w-2xl">
          <div className="font-bold text-sm text-slate-900 flex items-center gap-2">
            <FileCode className="w-4 h-4 text-emerald-600" />
            Content Architecture & Persistence Guide
          </div>
          <p className="text-slate-600 leading-relaxed">
            All edits take effect immediately across the site in your browser. To save your modifications permanently into the codebase, click <strong>"Export JSON Backup"</strong> and update the content files in your repository.
          </p>
        </div>
