    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-10 space-y-6 sm:space-y-12 lg:space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center relative z-10">
          ...
          <div className="lg:col-span-4 p-5 sm:p-7 rounded-2xl bg-white/[0.08] backdrop-blur-md border border-white/15 shadow-xl space-y-3 sm:space-y-4">
            ...
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-2xl sm:text-4xl font-black font-display text-[#FFC5DC]">
                ₹{course.price.toLocaleString('en-IN')}
              </span>
              {course.originalPrice && (
                <span className="text-xs sm:text-sm text-[#D4E2DF] line-through">
            ...
            <Button
              ...
              icon={<Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#FFC5DC]" />}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-start">
        ...
        <div className="lg:col-span-7 space-y-5 sm:space-y-8">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold font-display text-[#39340F] tracking-tight flex items-center gap-2">
              <BookOpen className="w-5 h-5 sm:w-6 sm:h-6 text-[#BC2639]" />
          ...
          <div className="space-y-2 sm:space-y-3">
            {course.syllabus.map((mod, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-xl sm:rounded-2xl border border-[#D4E2DF] bg-white overflow-hidden shadow-xs transition-all hover:border-[#BC2639]"
                >
                  <button
                    onClick={() => toggleModule(idx)}
                    className="w-full p-3 sm:p-5 text-left flex items-start justify-between gap-2.5 sm:gap-3 hover:bg-[#FAF6F8]/80 transition-colors"
                  >
                    <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                      <span className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-[#FDF2F5] text-[#39340F] text-[0.7rem] sm:text-xs font-black flex items-center justify-center shrink-0 mt-0.5">
                        {mod.sessionNumber}
                      </span>
                      <div className="min-w-0">
                        <h3 className="text-xs sm:text-base font-bold text-[#39340F] truncate sm:whitespace-normal">
                          {mod.title}
                        </h3>
                        <p className="text-[0.7rem] sm:text-xs text-[#404D68] mt-0.5">
                    ...
                    <ChevronDown
                      className={`w-4 h-4 sm:w-5 sm:h-5 text-[#404D68] transition-transform duration-200 shrink-0 ${
                        isOpen ? 'rotate-180 text-[#BC2639]' : ''
                      }`}
                    />
                  </button>
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="px-3.5 sm:px-5 pb-3.5 sm:pb-5 pt-1 border-t border-[#D4E2DF]/50 bg-[#FAF6F8]/50 space-y-2.5 sm:space-y-3">
                          <div>
                            <span className="text-[0.65rem] sm:text-[0.7rem] uppercase font-bold text-[#404D68] tracking-wider">
                              Session Objective:
                            </span>
                            <p className="text-xs sm:text-sm text-[#39340F] mt-0.5 leading-relaxed">
                              {mod.objective}
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <ul className="mt-1 space-y-1 text-xs text-[#404D68]">
                              {mod.topics.map((t, i) => (
                                <li key={i} className="flex items-center gap-2">
                                  <CheckCircle2 className="w-3.5 h-3.5 text-[#BC2639] shrink-0" />
                                  <span className="text-[0.72rem] sm:text-xs text-[#39340F]">{t}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
          ...
          {/* Learning Outcomes */}
          <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl bg-[#FDF2F5] border border-[#FFC5DC] space-y-2 sm:space-y-3">
            <h3 className="text-sm sm:text-base font-bold font-display text-[#39340F]">
              What You Will Confidently Master:
            </h3>
            <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-[#39340F]">
              {course.learningOutcomes.map((outcome, idx) => (
                <li key={idx} className="flex items-start gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#BC2639] shrink-0 mt-0.5" />
        ...
        <div className="lg:col-span-5 space-y-4 sm:space-y-6">
          {/* What's Included */}
          <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-[#D4E2DF] shadow-xs space-y-3 sm:space-y-4">
            <h3 className="text-sm sm:text-base font-bold font-display text-[#39340F] flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-[#BC2639]" />
              What’s Included in This Course:
            </h3>
            <ul className="space-y-2 sm:space-y-2.5 text-xs sm:text-sm text-[#39340F]">
              {course.whatIncluded.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2 sm:gap-2.5">
                  <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#BC2639] shrink-0 mt-0.5" />
          ...
          <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-[#D4E2DF] shadow-xs space-y-3 sm:space-y-4">
            ...
            <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-[#404D68]">
              {course.suitableFor.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#BC2639] shrink-0 mt-1.5" />
                  <span className="text-[0.75rem] sm:text-sm text-[#39340F]">{item}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {course.faqs && course.faqs.length > 0 && (
            <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-[#D4E2DF] shadow-xs space-y-3 sm:space-y-4">
              <h3 className="text-sm sm:text-base font-bold font-display text-[#39340F] flex items-center gap-2">
                <HelpCircle className="w-4 h-4 sm:w-5 sm:h-5 text-[#BC2639]" />
                Course Specific Questions:
              </h3>
              <div className="space-y-2 sm:space-y-3 text-xs">
                {course.faqs.map((faq, idx) => (
                  <div key={idx} className="pb-2.5 sm:pb-3 border-b border-[#D4E2DF]/60 last:border-none last:pb-0">
                    <h4 className="font-bold text-[#39340F] mb-0.5 text-[0.75rem] sm:text-xs">{faq.question}</h4>
                    <p className="text-[#404D68] leading-relaxed text-[0.72rem] sm:text-xs">{faq.answer}</p>
                  </div>
                ))}
              </div>
            </div>
